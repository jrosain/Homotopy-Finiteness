...
TODO: finish the proof =count-choice=.
#+begin_src ctt
  count-choice (A : U) (B : A -> U) (is-set-A : is-set A)
	       (k : Nat) (e : Equiv (Fin k) (Sg A B)) (is-finite-B : (x : A) -> is-finite (B x))
	       (H : (x : A) -> Prop-trunc (B x)) : (x : A) -> B x =
    \ x.
      let t : count (B x) = sorry (count (B x)) in
      count/global-choice
	( B x) t.1 t.2
	( H x)

  finite-choice (A : U) (B : A -> U) (is-set-A : is-set A)
		(is-finite-AB : is-finite (Sg A B)) (is-finite-B : (x : A) -> is-finite (B x))
		(H : (x : A) -> Prop-trunc (B x)) : Prop-trunc ((x : A) -> B x) =
    rec-Prop-trunc
      ( count (Sg A B))
      ( Prop-trunc/Prop ((x : A) -> B x))
      ( \ c. Prop-trunc/unit (count-choice A B is-set-A c.1 c.2 is-finite-B H))
      ( is-finite-AB)
#+end_src
