...
Of course, applying symmetry twice gives back the original equivalence.
#+begin_src ctt
  Equiv/sym/sym (A B : U) (e : Equiv A B)
    ...
    SgPath-prop
      ...
      ( refl (A -> B) (Equiv/map A B e))
#+end_src

We proceed as before by case analysis on e(inl x) = inr star + e(inl x) \neq inr star. As such, we have to: (i) constructively get the path e(inl x) = inl y and (ii) proceed as before to show that we have a left homotopy.
#+begin_src ctt
