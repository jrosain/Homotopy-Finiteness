...
#+end_src
We define the map using =precomp-Set-quotient=, and the inverse map using the recursor of set truncation.
#+begin_src ctt
  Set-trunc/is-set-quotient/map (X : U) (B : UU-Set)
                                   : (Set-trunc X -> Set/type B) -> (Set-quotient/identifiable-map-relation X (mere-eq/Prop X) B) =
  ...
  Set-trunc/is-set-quotient/inv-map (X : U) (B : UU-Set)
                                       : (Set-quotient/identifiable-map-relation X (mere-eq/Prop X) B) -> (Set-trunc X -> Set/type B) =
    \ t. rec-Set-trunc X B t.1 
#+end_src

These maps are inverse to each other by function extensionality.
#+begin_src ctt
  Set-trunc/is-set-quotient/right-htpy (X : U) (B : UU-Set) (t : Set-quotient/identifiable-map-relation X (mere-eq/Prop X) B)
                                          : Path (Set-quotient/identifiable-map-relation X (mere-eq/Prop X) B)
...
#+begin_src ctt
  Set-trunc/is-set-quotient (X : U) : is-set-quotient X (mere-eq/Prop X) (Set-trunc/Set X) (Set-trunc/relation X) =
    \ B.
      has-inverse/is-equiv
        ( Set-trunc X -> Set/type B)
        ( Set-quotient/identifiable-map-relation X (mere-eq/Prop X) B)
...
** Identity type on set truncation is equivalent to mere equality
