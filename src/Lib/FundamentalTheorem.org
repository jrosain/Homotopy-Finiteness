...
This result leads to another result that helps characterize structures. It is easy to show: we use the fact that we can make =y= and =z= commute in the types. Then, \Sigma (\Sigma A C) \cdot is contractible as \Sigma A C is contractible (using [[Prop/Contr.org][Sg/is-contr']]).
