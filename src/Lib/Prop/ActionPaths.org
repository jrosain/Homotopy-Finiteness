...
#+begin_src ctt
  ap/closure-bi-inv-inv-map (A B : U) (f : A -> B) (b : is-bi-inv A B f) (x y : A) (p : Path B (f x) (f y)) : Path A x y =
    ...
    comp-n A three-Nat x (g (f x)) (inv A (g (f x)) x (H x))
                     ...
                     y (H y)
#+end_src
We can show that it is the right-inverse of =ap f=. 
#+begin_src ctt
  ...
  --   \ p.
#+end_src
And also a left inverse. To do so, remark that (ap f (ap f^-1 p)) is (ap (f \circ f^-1) p). Then, using the naturality square is enough to conclude. The code is somewhat tedious, but what is happening is clear:
  ...
  ap/closure-bi-inv-left-htpy (A B : U) (f : A -> B) (b : is-bi-inv A B f) (x y : A)
    ...
    \ p.
      let g : B -> A = is-bi-inv/inv-map A B f b
          ...
          -- q2 : Path (Path A x y) (h (ap A A (\ z. (g (f z))) x y p)) (comp ) TODO
