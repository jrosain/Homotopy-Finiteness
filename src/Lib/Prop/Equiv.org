...
*** \Sigma (x: A) (\Sigma (y: B) (C x y)) and \Sigma (\Sigma A B) C(x, y)
Maps:
    #+begin_src ctt
...
*** Σ A B is A if B is contractible
Because if B is contractible, B ≃ Unit, and by previous results: Σ_(x:A)Unit ≃ A. The result is in the file [[file:Proposition.org][Lib/Prop/Proposition]].
...
*** \Sigma A B is (B a) whenever A is contractible
...
#+end_src
Hence, B a is equivalent to \Sigma A B.
#+begin_src ctt
...
See the result in the file [[file:Proposition.org][Lib/Prop/Proposition]] as it needs that Σ A B ≃ A whenever B is contractible.
