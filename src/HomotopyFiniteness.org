...
#+begin_src ctt
  im-Set-trunc-map/is-inj (A1 A2 B : U) (f : Coprod A1 A2 -> B) (e : Equiv (Coprod A1 A2) (Set-trunc B)) (is-set-A1 : is-set A1)
			  (H : Htpy' (Coprod A1 A2) (Set-trunc B) (\ x. Set-trunc/unit (f x)) (Equiv/map (Coprod A1 A2) (Set-trunc B) e))
			  (x y : A1) (p : Path (Set-trunc (im A1 B (\ z. f (inl z)))) (im-Set-trunc/map A1 A2 B f x) (im-Set-trunc/map A1 A2 B f y))
				: Path A1 x y =
    rec-Prop-trunc
...
#+end_src
That is, we have an equivalence between A1 and the set truncation of the image of f \circ inl.
#+begin_src ctt
  im-Set-trunc/Equiv (A1 A2 B : U) (f : Coprod A1 A2 -> B) (e : Equiv (Coprod A1 A2) (Set-trunc B)) (is-set-A1 : is-set A1)
		     (H : Htpy' (Coprod A1 A2) (Set-trunc B) (\ x. Set-trunc/unit (f x)) (Equiv/map (Coprod A1 A2) (Set-trunc B) e))
...
#+begin_src ctt
  is-htpy-finite/closed-Sg/z' (A : U) (B : A -> U) (is-htpy-finite-A : is-htpy-finite one-nat A)
			      (is-htpy-finite-B : (x : A) -> is-htpy-finite zero (B x))
			      (IH : (A : U) -> (B : A -> U) -> is-htpy-finite one-Nat A -> ((x : A) -> is-htpy-finite zero (B x))
					    -> (e : Equiv (Fin k) (Set-trunc A)) -> is-htpy-finite zero (Sg A B))
						: (k : Nat) -> (e : Equiv (Fin (suc k)) (Set-trunc A))
						  -> is-htpy-finite zero (Sg A B) = split
    zero -> \ e.
      is-htpy-finite/closed-Sg/base A B
    ...
    suc k -> \ e.
      rec-Prop-trunc
	( Sg (Fin (suc k) -> A) (\ f. Htpy' (Fin (suc k)) (Set-trunc A) (\ z. Set-trunc/unit (f (inl z))) (Equiv/map (Fin (suc k)) (Set-trun A) e)))
	( is-htpy-finite/Prop zero (Sg A B))
	( \ k.
	  let f : (Fin (suc k) -> A) = k.1
	      H : Htpy' (Fin (suc k)) (Set-trunc A) (\ z. Set-trunc/unit (f (inl z))) (Equiv/map (Fin (suc k)) (Set-trun A) e) = k.2
	      e' : Equiv (Coprod (im (Fin k) A (\ x. f (inl x))) (im Unit A (\ x. f (inr x)))) A = codomain-is-coproduct (Fin k) Unit A f e H
	  in
	  is-htpy-finite/closed-Equiv
	    ( Sg A B)
	    ( Coprod (Sg (im (Fin k) A (\ x. f (inl x))) (\ t. B (t.1))) (Sg (im Unit A (\ x. f (inr x))) (\ t. B (t.1))))
	    ( Equiv/trans
	      ( Sg A B)
	      ...
	      ( Coprod (Sg (im (Fin k) A (\ x. f (inl x))) (\ t. B (t.1))) (Sg (im Unit A (\ x. f (inr x))) (\ t. B (t.1))))
	      ( Sg/equiv-base' 
		( Coprod (Sg (im (Fin k) A (\ x. f (inl x))) (\ t. B (t.1))) (Sg (im Unit A (\ x. f (inr x))) (\ t. B (t.1)))) A B e')
		...
		( im-Set-trunc/Equiv 
	      ...
	      ( {- TODO: htpy finiteness of im Unit -})))
