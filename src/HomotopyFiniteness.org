...
#+begin_src ctt
  is-htpy-finite/closed-Sg/z : (k : Nat) -> (A : U) (B : A -> U) (is-htpy-finite-A : is-htpy-finite one-Nat A)
			       (is-htpy-finite-B : (x : A) -> is-htpy-finite zero (B x)) -> (e : Equiv (Fin k) (Set-trunc A))
    ...
    suc k -> \ A B is-htpy-finite-A is-htpy-finite-B e.
      rec-Prop-trunc
	( Sg (Fin (suc k) -> A) (\ f. Htpy' (Fin (suc k)) (Set-trunc A) (\ z. Set-trunc/unit (f z)) (Equiv/map (Fin (suc k)) (Set-trunc A) e)))
	( is-htpy-finite/Prop zero (Sg A B))
	( \ F.
	  let f : (Fin (suc k) -> A) = F.1
	      H : Htpy' (Fin (suc k)) (Set-trunc A) (\ z. Set-trunc/unit (f z)) (Equiv/map (Fin (suc k)) (Set-trunc A) e) = F.2
